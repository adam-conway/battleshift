...
describe ShipPlacer do
  before(:all) do
    ...
    it "exists when provided a board and ship" do
      subect = ShipPlacer.new(game: @game, ship: create(:ship), start_space: "A1", end_space: "A3", api_key: @player_1.api_key)
      expect(subject).to be_a ShipPlacer
    end

    xit "places the ship within a row with empty spaces" do
